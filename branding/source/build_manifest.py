...
def verify_links():
    ...
    for doc in ["README.md", "BRAND-GUIDE.md"]:
        candidates.extend(
            (ROOT, url) for url in re.findall(r"\[[^\]]*\]\(([^)]+)\)", (ROOT / doc).read_text())
        )
    # CSS font references are not represented as HTML links.
    candidates.extend(
        (ROOT, url)
        for url in re.findall(r"url\(['\"]([^'\"]+)['\"]\)", (ROOT / "index.html").read_text())
    )
...
def verify_assets():
    for kind in KINDS:
        for theme in THEMES:
            for ext in ["svg", "png"]:
                expect(
                    (ROOT / f"logos/muta-{kind}-{theme}.{ext}").is_file(),
                    f"Missing {kind}/{theme}/{ext}",
                )
    expect(len(list((ROOT / "logos").glob("*.svg"))) == 20, "Expected 20 logo SVGs")
    for path in (
        list((ROOT / "logos").glob("*.svg"))
        + list((ROOT / "icons").glob("*.svg"))
        + list((ROOT / "social").glob("*.svg"))
        + [ROOT / "brand-board.svg"]
    ):
        root = ET.parse(path).getroot()
        for node in root.iter():
            localname = node.tag.rsplit("}", 1)[-1]
            expect(
                localname not in ("text", "image", "script", "foreignObject"),
                f"Non-path artwork in {path.name}: {localname}",
            )
        ...
        if png.exists():
            with Image.open(png) as img:
                ...
                expect(
                    abs(img.height - img.width * view[3] / view[2]) <= 1,
                    f"Aspect ratio mismatch: {png.name}",
                )
    for path in (ROOT / "logos").glob("*.png"):
        with Image.open(path) as img:
            ...
            expect(
                ...
                f"Logo missing transparent surroundings: {path.name}",
            )
            expect(
                0 < box[0] < box[2] < img.width and 0 < box[1] < box[3] < img.height,
    ...
    for name, width in [
        ("muta-app-master", 1024),
        ("muta-touch", 180),
        ("muta-android-192", 192),
        ("muta-android-512", 512),
    ]:
        with Image.open(ROOT / f"icons/{name}.png") as img:
            expect(img.size == (width, width), f"Wrong icon dimensions: {name}")
            expect(
                img.convert("RGBA").getchannel("A").getextrema() == (255, 255),
                f"App icon not opaque: {name}",
            )
    with Image.open(ROOT / "icons/favicon.ico") as ico:
        ...
        for size in [16, 32, 48, 64]:
            with Image.open(ROOT / f"icons/muta-favicon-{size}.png") as png:
                ...
                expect(
                    ImageChops.difference(frame, png.convert("RGBA")).getbbox() is None,
...
def main():
    ...
    export_count = sum(
        path.suffix in (".svg", ".png", ".ico") and "reference" not in path.parts for path in files
    )
    manifest = {
        "brand": "Muta",
        "edition": 1,
        ...
        "counts": {
            "inventoried_files": len(entries),
            "artwork_exports": export_count,
            "logo_arrangements": 5,
            "color_treatments": 4,
            "collateral_layouts": 4,
        },
        "verified": [
            "SVG XML and self-contained path artwork",
            "Transparent logo PNGs and unclipped alpha bounds",
            "Social export dimensions",
            "Opaque app PNGs",
            "Exact optical ICO frame matches",
    ...
    print(
        f"Verified {export_count} artwork exports and {link_count} local links; inventoried {len(entries)} files."
    )
