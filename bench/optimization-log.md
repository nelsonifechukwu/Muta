...
|---|---|---|---|---|---|---|---|---|---|---|
| 2026-09-14 | Qwen3.5 judge-template candidate: independent-turn default, lexically gated one-pair follow-up memory, unconditional direct mode | stock llama.cpp b10520 `/apply-template`: 10 routing/compatibility assertions plus one real direct-generation check on macOS ARM; exact audit build and judge-quality battery pending | profiler fixed/capped 15 | stock full transcript; runtime can request thinking | unrelated turns absent by default; explicit follow-up retains one pair; `enable_thinking=true` cannot change rendered boundary; real response has no reasoning trace | profiler path unchanged; template is bypassed | tensor/RSS unchanged; metadata delta not yet measured | interactive quality not yet scored | **no score claim** | park until baked candidate passes the exact audit-build startup/render/generation battery and judged-prompt A/B |
| 2026-08-27 | Visualization V2 bearings/navigation: deterministic east/north solver plus one typed clockwise-angle SVG primitive | immutable 15-case first run vs post-implementation semantic + real-browser gate; 760/375/430/667 light/dark/reduced-motion matrices; full 200 + prior 50 regression; macOS dev host | not yet scored | held-out first run 0/15 correct; existing V2 source/resource baseline | 15/15 bearings + 200/200 baseline + 50/50 prior holdout; zero matrix overflow; synchronous UI 108,747 B, lazy V2 225,664 B, peak matrix JS heap 56,659,746 B | — | whole-process target RSS not measured | capability gate; no benchmark-accuracy substitution | **no score claim** | keep; typed general solver removes generic/fabricated bearing diagrams without a model or network dependency; packaging task must retain target RSS/thermal caveat |
| 2026-08-27 | Visualization Engine V2: lazy typed renderer, bounded sampling, render-on-demand Three.js, hard four-frame iframe LRU | 200-case real-browser gate + six desktop/mobile/theme/reduced-motion matrices on macOS; source/startup evidence in `docs/qa/visualization-v2-performance.json`; x86 target measurement deferred to the authorized Mac-package/target task | not yet scored | no V2 renderer / no V2 acceptance baseline | dev signal: 200/200 real renders; synchronous UI + measured source delta, lazy V2 module isolated from ordinary text chat, peak browser JS heap recorded by matrix | — | whole-process target RSS not measured | capability gate, not a benchmark accuracy replacement | **no score claim** | keep the bounded architecture; target x86 task must still measure process-tree RSS, frame pacing and thermals before any score claim |
