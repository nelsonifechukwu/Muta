...
raw="$campaign/raw"
out="$raw/stem-responses-gcp-remainder.jsonl"
events="$raw/stem-events-gcp-remainder.jsonl"
server="$repo/bench/.artifacts/llama.cpp-b10175/build/bin/llama-server"

mkdir -p "$raw" "$campaign/stem-server-logs-gcp-remainder"
...
if pgrep -x llama-bench >/dev/null || pgrep -x llama-server >/dev/null || \
    ...
    echo "unexpected model or profiler process before STEM remainder" >&2
    exit 1
fi
...
bench/.venv-profiler/bin/python -m bench.run_stem_prompt_suite \
    ...
    --out "$out" \
    --events "$events" \
    --logs "$campaign/stem-server-logs-gcp-remainder" \
    --port 18080 \
    --threads 4
