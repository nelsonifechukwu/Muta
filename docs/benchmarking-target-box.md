...
| 6 cores / 12 threads | cpuset of 6 physical cores + their SMT siblings (real core boundaries, like the box has); falls back to a cfs **quota** of the same logical count where the runtime can't pin (quota throttles instead of pinning — the report records which one it got) | good / fair |
