...
- Instructions said step 4 (`madvise(C, MADV_WILLNEED)` from a second thread, main thread polls mincore every 50 ms / 10 s timeout) should "report effective prefetch bandwidth and whether the madvise CALL itself blocked" (`task-A3-brief` resolutions, step 4) — implicitly assuming `WILLNEED` succeeds within the window. Measured (`scripts/stream_probe.c` step 4, container run): the call itself is non-blocking (0.13 ms, confirming R10's "does not block" expectation) but mincore **never reached 95%** within the full 10 s timeout — only 0.5% resident, +663,552 B added to `memory.stat`'s `file` counter out of a 256 MiB region. A follow-up diagnostic (ad hoc Python `mmap.madvise(MADV_WILLNEED)`, same `cgrun 3g` container class, not committed — throwaway `/tmp` script, deleted after) confirms this is not a slow-ramp effect that just needed more time: `memory.current` rose ~1.09 MB in the first ~1 s after the call and then flatlined for a further 20+ s of polling; a subsequent explicit touch of the same region still paid the full cold-read cost (195 ms for 256 MiB ≈ 1.3 GiB/s, i.e. disk speed, not RAM speed) — proof the region was genuinely never warmed beyond that small initial burst. Did: documented `MADV_WILLNEED`'s background readahead as effectively inert beyond ~1 MiB in this emulated-aarch64 Docker Desktop VM (`docs/DISCOVERY.md` S0.3, step 4) and recommend an explicit touch/read loop (measured 1.1–2.4 GiB/s across runs, step 1) as the residency manager's actual prefetch mechanism instead of `MADV_WILLNEED`. Does not affect the go/no-go verdict (step 3's primary eviction primitive passed cleanly independent of step 4).
- Consequence of the above: step 5 ("Plan-B evict validation ... on region C (now resident)") assumes step 4 successfully populated C. Since it did not, step 5's measured delta (-1,310,720 B, ~1.25 MiB, not -256 MiB) reflects evicting whatever small sliver step 4 actually loaded, not a real test of the `MAP_FIXED` remap fallback against a genuinely-resident region. Logged as a known confound in `docs/DISCOVERY.md` S0.3 rather than silently reporting a misleadingly-small "Δ" as if the fallback were weak — it wasn't tested under the intended precondition. Not re-run with a touch-warmed C because the go/no-go didn't need it: step 3 (the primary primitive) already reclaimed exactly 100% of its region.
