...
- gguf import: task-A2-brief.md's resolution said `scripts/layer_sizes.py` should `sys.path.insert(0, 'llama.cpp/gguf-py')` only "if plain `import gguf` fails" -> found: a pip-installed `gguf==0.19.0` sits in `.venv/lib/python3.12/site-packages/gguf/` (from an earlier editable-install of this tree that pip actually resolved as a normal, non-editable package — see `pip show gguf`) and a bare `import gguf` **succeeds** against that package, never raising `ImportError`, so the brief's own fallback condition would never fire and the script would silently read tensors with the wrong, older `gguf-py`. The two versions differ in `gguf_reader.py`: this tree's copy (`llama.cpp/gguf-py/gguf/gguf_reader.py:270-271`) added a `GGML_MAX_DIMS` bounds check and (`:332-335`) switched `n_elements` from `np.prod(dims)` (silently overflow-prone on `uint64`) to a pure-Python-int accumulation; the pip package has neither -> did: `scripts/layer_sizes.py:24-30` unconditionally prepends `llama.cpp/gguf-py` to `sys.path[0]` before importing `gguf`, so this tree's copy always wins regardless of what else is installed (verified via `gguf.__file__` pointing into the worktree, not `site-packages`). No numeric difference for this task's tensors (all well within the safe integer range either way), but it is the only binding that actually satisfies the brief's "this tree's version" requirement.
