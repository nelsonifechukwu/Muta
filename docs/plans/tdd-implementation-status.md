...
| **T2** `fetch_models.sh` + MANIFEST | partial | hashes verify twice | Manifest build/verify implemented and tested (single-bit flip, truncation, missing file, license policy). Downloads not run: the competition core/mmproj/draft revisions are unpinned pending D1. |
