...
The campaign wrapper (source path: `run_judges_suite.sh`; not included in the published bundle) attempts to restore both services on exit. Its journal records the Ops Agent start command at 20:16:21; `google-cloud-ops-agent.service` is active.
...
All five source files exist in the checkout. `ui/dist/brand` and `ui/dist/muta-icon.svg` are absent. The native exporter (source path: `../../../scripts/export_native_linux.py`; not included in the published bundle) discovers only top-level `.css`, `.html`, and `.js` files, then overlays that inventory before validating the updated HTML. Consequently, the HTML is copied but its SVG dependencies are omitted. Startup exits at this validation step before launching the engine.
