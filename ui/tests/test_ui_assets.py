...
def test_sidebar_delete_is_confirmation_only_with_keyboard_focus_management():
    ...
    assert "stopGeneration(activeJob, { drain: false })" in confirm
    assert "discardQueue(pending.conversation.id" in confirm
    assert confirm.index("discardQueue") < confirm.index("stopGeneration")
...
def test_partial_failure_and_stop_have_distinct_recoverable_terminal_lifecycles():
    ...
    assert "activeJob.pendingRegen = null" in deletion
    assert "stopGeneration(activeJob, { drain: false })" in deletion
    assert 'if (!stopped) throw new Error("generation stop failed")' in deletion
