  ...
  try {
    ...
    for (const c of body.conversations) {
      ...
      del.addEventListener("click", async (ev) => {
        ev.stopPropagation();
        // Clear every client-side successor before Stop can settle the current job. Otherwise
        // finishGeneration may dispatch a queued follow-up into the conversation in the small
        // gap before this DELETE, creating a new server job against a thread being erased.
        discardQueue(c.id, { announce: false });
        if (backgroundJob) {
          backgroundJob.pendingRegen = null;
          const stopped = await stopGeneration(backgroundJob, { drain: false });
          if (!stopped) return;
        }
